...
import { patchRenewalSubscription } from "../utilities/api/api"
...
const SubscriptionsPurchased = () => {
   ...
   const renewalHandler = async (subscriptionData: any, subscriptionId: string) => {
      try {
       
         const response = await patchRenewalSubscription(subscriptionData, subscriptionId)
         
         if (response.data?.data?.paymentId) {
         navigate(`/payment?paymentId=${response.data?.data.paymentId}`, {
   ...
   const activeSubscriptions =data?.filter((sub: any) => sub.data.data.status === 'active') || []

   return ( <ContentBox label=' اشتراک‌های خریداری‌شده'>
         <TableContainer
            sx={{
            ...
            <Table>
               ...
               <TableBody>
                  {activeSubscriptions.map((sub, index) => {
                     const daysRemaining = calculateRemainingDays(sub.data.expireDate)
                     return (
