...
export const addClassStudent = async (data: any,classeId:string) => {
  return await postData<BaseAddResponseEntity>(classStudentsEndpoints.addClasseStudent(classeId),data)
}
...
export const listSummaryClasses = async (educationGradeId: string) => {
  return await getAllData<BaseIdDataEntity<listSummaryClassesEntity>[]>(
